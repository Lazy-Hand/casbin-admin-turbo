...
import { boolean, foreignKey, integer, jsonb, pgEnum, pgTable, primaryKey, serial, text, timestamp } from 'drizzle-orm/pg-core'

export const ResourceType = pgEnum('ResourceType', ['menu', 'api', 'button'])

export const MenuType = pgEnum('MenuType', ['menu', 'page', 'link', 'iframe', 'window', 'divider', 'group'])

export const FileType = pgEnum('FileType', ['IMAGE', 'VIDEO', 'AUDIO', 'DOCUMENT', 'ARCHIVE', 'FILE'])
...
export const Timer = pgTable('sys_timer', {
	...
	nextRunAt: timestamp('nextRunAt', { precision: 3 }),
	createdAt: timestamp('createdAt', { precision: 3 }).notNull().defaultNow(),
	updatedAt: timestamp('updatedAt', { precision: 3 }).notNull(),
	deletedAt: timestamp('deletedAt', { precision: 3 }),
	createdBy: integer('createdBy'),
...
export const TimerExecutionLog = pgTable('sys_timer_execution_log', {
	id: serial('id').notNull().primaryKey(),
...
export const LoginLog = pgTable('sys_login_log', {
	id: serial('id').notNull().primaryKey(),
...
export const DictItem = pgTable('sys_dict_item', {
	id: serial('id').notNull().primaryKey(),
...
export const Dept = pgTable('sys_dept', {
	id: serial('id').notNull().primaryKey(),
	name: text('name').notNull(),
