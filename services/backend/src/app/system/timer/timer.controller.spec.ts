...
describe('TimerController ability metadata', () => {
  it('protects mutating endpoints with timer permissions', () => {
    const createRules = Reflect.getMetadata(
      CHECK_ABILITY_KEY,
      TimerController.prototype.create,
    );
    const updateRules = Reflect.getMetadata(
      CHECK_ABILITY_KEY,
      TimerController.prototype.update,
    );
    const removeRules = Reflect.getMetadata(
      CHECK_ABILITY_KEY,
      TimerController.prototype.remove,
    );
    const runRules = Reflect.getMetadata(
      CHECK_ABILITY_KEY,
      TimerController.prototype.run,
    );
