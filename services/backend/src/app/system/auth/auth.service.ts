...
export class AuthService {
  ...
  private async refreshSessionActivity(
    ...
    const shouldRefreshSession =
      sessionTtl === -1 || sessionTtl === -2 || sessionTtl <= this.SESSION_REFRESH_THRESHOLD;
    const shouldRefreshUserCache =
      userTtl === -1 || userTtl === -2 || userTtl <= this.SESSION_REFRESH_THRESHOLD;
  ...
  private groupAuthUser(
    ...
    for (const row of rows) {
      ...
      if (
        ...
        (!permissionTypes ||
          (row.permission.resourceType && permissionTypes.includes(row.permission.resourceType))) &&
